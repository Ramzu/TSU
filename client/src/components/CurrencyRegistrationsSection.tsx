...
interface CurrencyRegistration {
  id: string;
  companyName: string;
  contactEmail: string;
  contactPhone?: string;
...
export default function CurrencyRegistrationsSection() {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [selectedRegistration, setSelectedRegistration] = useState<CurrencyRegistration | null>(null);
  const [adminNotes, setAdminNotes] = useState("");
  const [selectedStatus, setSelectedStatus] = useState<string>("");
  const [isViewModalOpen, setIsViewModalOpen] = useState(false);

  // Fetch currency registrations
  const { data: registrations = [], isLoading } = useQuery<CurrencyRegistration[]>({
    queryKey: ["/api/currency-registrations"],
  });

  // Update registration status mutation
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, status, notes }: { id: string; status: string; notes: string }) => {
      return apiRequest("PUT", `/api/currency-registrations/${id}`, {
        status,
        adminNotes: notes,
      });
    },
  ...
  const handleViewRegistration = (registration: CurrencyRegistration) => {
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Loading Currency Registrations...
          </CardTitle>
        </CardHeader>
        <CardContent>
  ...
  return (
    <>
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <CreditCard className="h-5 w-5 text-tsu-green" />
        ...
        <CardContent>
          {totalCount === 0 ? (
            <div className="text-center py-8" data-testid="no-currency-registrations">
              <CreditCard className="h-12 w-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-500">No currency registrations found</p>
            </div>
          ) : (
              ...
              {registrations.map((registration) => {
                ...
                return (
                  <div
                    ...
                    data-testid={`currency-registration-${registration.id}`}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <h4 className="font-semibold text-gray-900" data-testid={`currency-company-${registration.id}`}>
                            {registration.companyName}
                          </h4>
                          <Badge className={statusColors[registration.status]} data-testid={`currency-status-${registration.id}`}>
                        ...
                        <div className="grid md:grid-cols-2 gap-2 text-sm text-gray-600">
                          <div className="flex items-center gap-1">
                            ...
                            {registration.currencyType} - {registration.amount}
                          </div>
                          <div className="flex items-center gap-1">
                      ...
                      <div className="ml-4">
                        <Button
                          ...
                          data-testid={`button-view-currency-${registration.id}`}
                        >
      ...
      <Dialog open={isViewModalOpen} onOpenChange={setIsViewModalOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto" data-testid="currency-registration-modal">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-tsu-green">
              Currency Registration Details
            </DialogTitle>
          </DialogHeader>
          ...
          {selectedRegistration && (
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="grid md:grid-cols-2 gap-3 text-sm">
                  <div>
                    <span className="font-medium">Currency Type:</span> {selectedRegistration.currencyType}
                  </div>
                  <div>
                    <span className="font-medium">Amount:</span> {selectedRegistration.amount}
                  </div>
                </div>
                {selectedRegistration.purpose && (
                  <div className="mt-3">
                    <span className="font-medium">Purpose:</span>
                    <p className="mt-1 text-gray-700 whitespace-pre-wrap">{selectedRegistration.purpose}</p>
                  </div>
                )}
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <Select value={selectedStatus} onValueChange={setSelectedStatus}>
                      <SelectTrigger data-testid="select-currency-status">
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      rows={3}
                      data-testid="textarea-currency-admin-notes"
                    />
                  </div>
                  <div className="flex justify-end">
                    <Button
                      ...
                      data-testid="button-update-currency-status"
                    >
